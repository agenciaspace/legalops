...
- `community_discussion_summaries`: legacy weekly forum summaries with their existing Pro policies.
- `club_whatsapp_summaries`, `club_whatsapp_summary_schedule`: daily WhatsApp digests and availability, readable by active Club members.
...
Club welcome/invitation emails use Resend (`RESEND_API_KEY`, sender
`contato@legalops.club`); Supabase Auth uses Resend SMTP. Branded SMTP templates
are in `supabase/templates`, generated by `node scripts/build-auth-email-templates.mjs`.
Cloudflare Email Service handles Work outbound mail, signup fallback and inbound
aliases through the Cloudflare webhook route.
...
`automation/whatsapp-summary/runner.py` reads the existing local instance mirror for the
community group and calls `/api/cron/whatsapp-summary` with a dedicated bearer secret.
It publishes and sends one fixed 24-hour digest daily at 18:00 Brasília. The independent
systemd timer retries unfinished periods every five minutes; it does not restart gateways
or re-enable legacy group automations. See the automation README for deployment and state.
