...
export function StatusDropdown({ entryId, currentStatus }: StatusDropdownProps) {
  ...
  return (
    <select
      ...
      disabled={saving}
      className="text-sm border border-[#1A1A1A]/20 rounded-xl px-2 py-1 focus:outline-none focus:ring-2 focus:ring-[#FF6A00] focus:border-[#FF6A00] disabled:opacity-50"
    >
      {OPTIONS.map(opt => (
