  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-semibold text-slate-900">{t.emailPage.title}</h1>
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_1.2fr]">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-sm font-semibold text-slate-900">{t.emailPage.planRules}</h2>
          <div className="mt-4 space-y-3 text-sm text-slate-600">
            <div className="flex items-center justify-between">
              <span>{t.emailPage.currentTier}</span>
              <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-slate-700">
                {initialTier}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span>{t.emailPage.activeAliases}</span>
              <span className="font-medium text-slate-900">
                {activeAliases.length} / {policy.maxActiveAliases}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span>{t.emailPage.randomAliases}</span>
              <span className="font-medium text-slate-900">
                {policy.allowsRandomAliases ? t.emailPage.enabled : t.emailPage.unavailable}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span>{t.emailPage.customAliases}</span>
              <span className="font-medium text-slate-900">
                {policy.allowsCustomAliases ? t.emailPage.enabled : t.emailPage.paidOnly}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-sm font-semibold text-slate-900">{t.emailPage.createAlias}</h2>
          <p className="mt-1 text-sm text-slate-500">
            {t.emailPage.remainingSlots(remainingSlots)}
          </p>
          ...
          <div className="mt-4 flex flex-col gap-3">
            ...
            >
              {submitting === 'random' ? t.emailPage.creatingRandom : t.emailPage.createRandom}
            </button>
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                {submitting === 'custom' ? t.emailPage.creatingCustom : t.emailPage.createCustom}
              </button>
            </div>
          </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-5 py-4">
          <h2 className="text-sm font-semibold text-slate-900">{t.emailPage.provisionedAliases}</h2>
        </div>

        {aliases.length === 0 ? (
          <div className="px-5 py-8 text-sm text-slate-500">
            {t.emailPage.noAliases}
          </div>
        ) : (
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-5 py-4">
          <h2 className="text-sm font-semibold text-slate-900">{t.emailPage.recentMailbox}</h2>
        </div>

        {messages.length === 0 ? (
          <div className="px-5 py-8 text-sm text-slate-500">
            {t.emailPage.noMessages}
          </div>
        ) : (
          <div className="divide-y divide-slate-200">
