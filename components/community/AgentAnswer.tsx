...
export const AgentAnswer = memo(function AgentAnswer({ answer }: { answer: string }) {
 const { t } = useClubLanguage()
  ...
  </div><button type="button" aria-label={copied ? t("Resposta copiada") : t("Copiar resposta")} onClick={async () => {
    try { await navigator.clipboard.writeText(answer); setCopied(true); setError('') }
    catch { setError(t("Não foi possível copiar. Selecione o texto da resposta.")) }
  }} className="mt-2 inline-flex min-h-11 items-center gap-2 rounded-lg px-2 text-xs text-[#817A73] hover:bg-[#F5F1E8]">{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{copied ? t("Copiado") : t("Copiar")}</button>{error && <p role="status" className="text-xs text-red-700">{error}</p>}</>
})
