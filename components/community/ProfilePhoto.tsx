...
export function ProfilePhoto({userId,path,name}:{userId:string;path:string|null;name:string}) {
 ...
 async function upload(file:File|undefined) {
  ...
  try{
   ...
   setRemoved(false);setPreview(data.url);setMessage(t("Foto salva. Ela já aparece nas suas publicações."));router.refresh()
 ...
 async function remove() {
  ...
  try {const response=await fetch('/api/club/avatar',{method:'DELETE'});const data=await response.json();if(!response.ok)throw new Error(data.error);setRemoved(true);setPreview(null);setMessage(t("Foto removida."));router.refresh()}
  catch(error){setFailed(true);setMessage(error instanceof Error?error.message:t("Não conseguimos remover a foto."))}
  finally{setBusy(false)}
 }
 return <section className="mb-6 rounded-xl bg-[#F5F1E8] p-4"><h2 className="text-lg font-semibold">{t("Sua foto na comunidade")}</h2><div className="mt-4 flex flex-wrap items-center gap-4">{preview?<img src={preview} alt={t("Foto de {name}",{name})} className="h-20 w-20 rounded-full object-cover"/>:<MemberAvatar userId={userId} path={removed?null:path} name={name} size="h-20 w-20"/>}<label className="min-w-0 flex-1 text-sm font-medium">{busy?t("Salvando foto…"):!removed&&(path||preview)?t("Trocar foto"):t("Adicionar sua foto")}<input type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={event=>void upload(event.target.files?.[0])} className="mt-2 block min-h-12 w-full max-w-full text-sm file:mr-3 file:rounded-lg file:border-0 file:bg-[#24231F] file:px-3 file:py-3 file:text-sm file:text-white"/></label></div>{!removed&&(path||preview)?<button type="button" disabled={busy} onClick={()=>void remove()} className="mt-2 min-h-11 text-sm underline">{t("Remover foto")}</button>:null}<p className="mt-3 text-xs leading-6 text-[#625E59]">{t("JPG, PNG ou WebP, até 10 MB. Sua foto é exibida aos membros do Club, junto do seu nome e local de trabalho.")}</p>{message?<p role={failed?'alert':'status'} className={`mt-3 text-sm ${failed?'text-red-700':'text-green-800'}`}>{t(message)}</p>:null}</section>
}
