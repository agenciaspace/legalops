...
export function AgentBubble({hasPro}:{hasPro:boolean}){
 ...
 return <><button ref={trigger} type="button" onClick={()=>{setStarted(true);setOpen(true)}} aria-label={t("Abrir meu agente")} aria-expanded={open} aria-controls="personal-agent-dialog" className="agent-bubble fixed right-4 z-40 flex min-h-14 items-center gap-2 rounded-full bg-[#24231F] px-5 text-sm font-semibold text-white shadow-lg"><MessageCircle className="h-5 w-5"/>{t("Meu agente")}</button>
 {started?<div hidden={!open} style={viewport?{top:viewport.top,height:viewport.height,bottom:'auto'}:undefined} className="agent-overlay fixed inset-0 z-[70] bg-black/25" onClick={dismiss}><div ref={dialog} id="personal-agent-dialog" role="dialog" aria-modal="true" aria-label={t("Meu agente")} className={`agent-dialog absolute inset-0 flex flex-col bg-white sm:inset-auto sm:bottom-4 sm:right-4 sm:rounded-2xl sm:border sm:border-[#CEC8BD] sm:shadow-xl ${expanded?'sm:h-[min(900px,94dvh)] sm:w-[min(960px,calc(100vw-2rem))]':'sm:h-[min(780px,90dvh)] sm:w-[min(560px,calc(100vw-2rem))]'}`} onClick={e=>e.stopPropagation()}><div className="flex shrink-0 items-center justify-between border-b border-[#E6DED0] px-4 pt-[env(safe-area-inset-top)]"><span className="text-sm font-semibold">{t("Meu agente")}</span><div className="flex items-center"><button type="button" onClick={()=>setExpanded(value=>!value)} aria-label={expanded?t("Reduzir conversa"):t("Ampliar conversa")} className="hidden min-h-12 min-w-12 items-center justify-center sm:flex">{expanded?<Minimize2 className="h-4 w-4"/>:<Maximize2 className="h-4 w-4"/>}</button><button ref={close} type="button" onClick={dismiss} className="flex min-h-12 min-w-12 items-center justify-center" aria-label={t("Fechar agente")}><X className="h-5 w-5"/></button></div></div>{hasPro?<PersonalAgent embedded/>:<div className="p-6"><h2 className="text-xl font-semibold">{t('Uma conversa para seus assuntos.')}</h2><p className="mt-3 text-sm leading-7">{t('Resuma suas interações, descubra posts para acompanhar e encontre Bench relacionado ao seu perfil. O agente pessoal faz parte do Pro.')}</p><Link href="/club/checkout" className="mt-5 inline-flex min-h-12 items-center rounded-xl bg-[#24231F] px-4 text-sm font-semibold text-white">{t('Consultar disponibilidade')}</Link></div>}</div></div>:null}</>
}
