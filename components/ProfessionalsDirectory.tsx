...
const professionalTypeLabels: Record<string, Record<string, string>> = {
  pt: {
    law_firm: 'Escritorio',
    legal_dept: 'Juridico In-house',
    public_sector: 'Setor Publico',
    freelance: 'Freelance',
    other: 'Outro',
  },
  en: {
...
export function ProfessionalsDirectory({ professionals }: { professionals: Professional[] }) {
  ...
  const typeLabels = professionalTypeLabels[locale] ?? professionalTypeLabels.pt
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">{t.professionals.title}</h1>
          <p className="mt-1 text-sm text-slate-500">{t.professionals.subtitle}</p>
        </div>
      </div>
      ...
      <div className="mt-6">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-medium text-slate-900">
                        {p.full_name || t.professionals.professional}
                      </h3>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {p.professional_type && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">
                    ...
                    {typeLabels[p.professional_type] || p.professional_type}
                  </span>
                )}
                {p.years_experience && (
                  <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">
                    {t.professionals.yearsExp(p.years_experience)}
                  </span>
                )}
              </div>
        ...
        {filtered.length === 0 && (
          <p className="mt-8 text-center text-sm text-slate-500">
            {search ? t.professionals.noResultsSearch : t.professionals.noResults}
          </p>
        )}
      </div>
    </div>
