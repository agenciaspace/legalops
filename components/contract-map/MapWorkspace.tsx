...
export function MapWorkspace({ project = 'migration' }: { project?: 'migration' | 'playbook' }) {
  const playbook = project === 'playbook'
  const initialSection = playbook ? 'playbook-contexto' : 'contexto'
  const api = `/api/community/contract-map?project=${project}`
  ...
  useEffect(() => {
    setSelected(new URLSearchParams(window.location.search).get('section') || initialSection)
  ...
  const archived = !playbook && (LEGACY_MAP_SECTION_IDS as readonly string[]).includes(selected)
  const sections = data?.sections.filter(item => playbook ? item.journey === 'open-playbook' : archived ? (LEGACY_MAP_SECTION_IDS as readonly string[]).includes(item.id) : MIGRATION_STAGES.some(stage => stage.id === item.id)) ?? []
  ...
  return <main className="mx-auto min-w-0 w-full max-w-7xl px-4 py-6 sm:px-6">
    ...
    <div className="mt-4 flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-semibold">{playbook ? 'Playbook aberto' : archived ? 'Ciclo contratual · arquivo' : 'Migração de CLM'}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#625E59]">{playbook ? 'Construa posições de negociação, alternativas, limites e aprovações com a comunidade. Membros propõem melhorias; membros-lead revisam e publicam.' : 'Um caminho para entender seu jurídico, escolher uma solução e fazer a mudança funcionar. Membros propõem melhorias; membros-lead revisam e publicam.'}</p></div><a className="text-sm underline" href={playbook ? "https://legalops.dev/playbook/" : "https://legalops.dev/mapa-contratos/"} target="_blank" rel="noreferrer">Ver publicação ↗</a></div>
    ...
    {!section ? <div className="mt-6"><p>{error ? 'O documento não carregou.' : 'Carregando documento…'}</p><button className={button} onClick={() => load().catch(e => setError(e.message))}>Tentar novamente</button></div> : <>
      {playbook ? <p className="mt-4 text-sm leading-6 text-[#625E59]">Comece pelo contexto e acrescente temas nas posições de negociação. Use títulos, listas e tabelas para organizar o playbook. A estrutura inicial ainda não contém posições contratuais aprovadas.</p> : archived ? <p className="mt-5 text-sm">Esta é a estrutura anterior do ciclo contratual, com seu histórico preservado. <button className="font-semibold underline" onClick={() => navigate('contexto')}>Abrir Migração de CLM →</button></p> : <>
      ...
      <div className="mt-6 flex flex-wrap items-end gap-3"><label className="min-w-0 flex-1 text-sm font-semibold">{playbook ? 'Seção do playbook' : 'Etapa da jornada'}<select disabled={!!mode || busy} value={section.id} onChange={e => navigate(e.target.value)} className="mt-2 block min-h-12 w-full rounded-xl border border-[#CEC8BD] bg-white px-3">{sections.map((item,index) => <option key={item.id} value={item.id}>{index+1}. {item.title}</option>)}</select></label><button className={button} disabled={busy} onClick={() => load().then(() => setMessage('Documento atualizado. Rascunhos em edição foram preservados.')).catch(e => setError(e.message))}>Atualizar</button></div>
      ...
      {tab === 'discussion' && <section className="mt-5 space-y-4">
        ...
        </article>)}<p className="text-xs text-[#625E59]">São exibidas as contribuições mais recentes (até 500 neste projeto).</p>
      </section>}
      {tab === 'history' && <section className="mt-5 space-y-3">{data!.revisions.filter(item => item.section_id === section.id).map(item => <details key={item.version} className="rounded-xl border border-[#CEC8BD] bg-white p-4"><summary className="cursor-pointer text-sm"><strong>Versão {item.version}</strong> · {author(item.editor_id)} · {new Date(item.created_at).toLocaleDateString('pt-BR')}<span className="mt-1 block text-[#625E59]">{item.note}</span></summary><div className="mt-4"><MapContent content={item.content} /></div></details>)}<p className="text-xs text-[#625E59]">Últimas 100 revisões deste projeto. As anteriores permanecem preservadas.</p></section>}
    </>}
    {!playbook && !archived && !mode && <details className="mt-8 border-t border-[#CEC8BD] pt-4 text-xs text-[#625E59]"><summary className="cursor-pointer">Sobre esta construção aberta</summary><p className="mt-3 leading-6">Roteiro adaptável, sem nota universal de maturidade. Registre o que não se aplica e proponha variáveis específicas do seu contexto. A jornada evolui com a comunidade.</p><button className="mt-3 min-h-11 underline" onClick={() => navigate('solicitacao')}>Consultar o ciclo contratual anterior e seu histórico</button></details>}
  </main>
}
