...
export function MapWorkspace() {
  ...
  const [comment,setComment]=useState<{quote?:string;parent?:string}|null>(null)
  ...
  useEffect(() => {
    ...
    if(window.location.hash.startsWith('#contribution-'))setTab('discussion')
    load().catch(error => setError(error.message))
  }, [])
  useEffect(() => {
    const select = () => { const selection=window.getSelection(); const element=approvedRef.current; if(element&&selection?.anchorNode&&selection.focusNode&&element.contains(selection.anchorNode)&&element.contains(selection.focusNode))setSelectedQuote(selection.toString().trim().slice(0,1000)) }
    document.addEventListener('selectionchange',select);return()=>document.removeEventListener('selectionchange',select)
  },[])
  useEffect(()=>{if(data&&tab==='discussion'&&window.location.hash.startsWith('#contribution-'))document.getElementById(window.location.hash.slice(1))?.scrollIntoView({block:'center'})},[!!data,tab])
  ...
  function navigate(id: string) { setSelected(id); setNote(''); setReviewing(null); setSelectedQuote(''); setComment(null); setError(''); setMessage(''); window.history.replaceState(null, '', `?section=${id}`) }
  ...
  function start(action: 'suggest' | 'publish') { if (!section) return; setDraft(section.content); setBase(section.version); setMode(action); setEditorReady(false); setEditorMentions([]); setNoteMentions([]); setPreservedDraft(null); setNote(''); setLicense(false); setError(''); setMessage('') }
  return <main className="mx-auto min-w-0 w-full max-w-5xl px-4 py-6 sm:px-6">
    ...
    {!section ? <div className="mt-6"><p>{error ? 'O mapa não carregou.' : 'Carregando mapa…'}</p><button className={button} onClick={() => load().catch(e => setError(e.message))}>Tentar novamente</button></div> : <>
      ...
      {tab === 'document' && <section className="mt-5"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h2 className="font-semibold">{section.title} <span className="text-xs font-normal text-[#625E59]">· versão {section.version}</span></h2>{!mode && <div className="flex flex-wrap gap-2"><button className={button} onClick={() => start('suggest')}>Sugerir alteração</button>{data!.isLead && <button className={`${button} bg-[#24231F] text-white`} onClick={() => start('publish')}>Editar e publicar</button>}</div>}</div>
        {!mode ? <div><div ref={approvedRef} onMouseUp={event=>{const selection=window.getSelection();if(selection?.anchorNode&&selection.focusNode&&event.currentTarget.contains(selection.anchorNode)&&event.currentTarget.contains(selection.focusNode))setSelectedQuote(selection.toString().trim().slice(0,1000))}} onKeyUp={event=>{const selection=window.getSelection();if(selection?.anchorNode&&event.currentTarget.contains(selection.anchorNode))setSelectedQuote(selection.toString().trim().slice(0,1000))}} className="rounded-2xl border border-[#CEC8BD] bg-white p-5 sm:p-7"><MapContent content={section.content} /></div><div className="mt-2 flex flex-wrap items-center gap-3"><p className="text-xs text-[#625E59]">Selecione um trecho para comentar. Nas conversas, use @ para chamar alguém.</p>{selectedQuote&&<button className={button} onClick={()=>setComment({quote:selectedQuote})}>Comentar trecho selecionado</button>}</div></div> : <form onSubmit={e => { e.preventDefault(); send({ action: mode, section: section.id, version: base, content: draft, note, license, mentions: Array.from(new Set([...editorMentions,...noteMentions])).slice(0,10) }, mode === 'publish' ? 'Nova versão publicada no mapa público.' : 'Proposta enviada para revisão pelos membros-lead.') }} className="space-y-4">
          ...
          <MapEditor content={draft!} sectionId={section.id} version={base} userId={data!.userId} userName={data!.userName??'Membro'} onChange={setDraft} onQuote={quote=>setComment({quote})} onMentions={setEditorMentions} onReady={setEditorReady} />
      ...
      {comment&&<CommentComposer modal section={section.id} version={mode?base:section.version} quote={comment.quote} parent={comment.parent} onClose={()=>setComment(null)} onSent={()=>{setMessage('Comentário publicado. O rascunho continua em edição.');load().catch(e=>setError(e.message))}}/>}
    </>}
