  ...
  return <section className={`personal-agent ${embedded?'agent-embedded':''} mx-auto flex w-full max-w-5xl flex-col px-4 sm:px-6`} aria-label={t("Seu agente pessoal")}>
    ...
    <div ref={scroll} onScroll={()=>{const el=scroll.current;if(el){nearBottom.current=el.scrollHeight-el.scrollTop-el.clientHeight<80;setShowLatest(!nearBottom.current)}}} className="min-h-0 flex-1 overflow-y-auto overscroll-contain py-4" aria-label={t("Histórico da conversa")} role="log" aria-live="polite" tabIndex={0}>
      {loading?<p role="status" className="text-sm text-[#625E59]">{t("Carregando sua conversa…")}</p>:<>
        ...
        {!turns.length&&!pendingQuestion&&loaded?<div className="mx-auto flex min-h-full max-w-xl flex-col justify-center py-6"><h2 className="text-center text-2xl font-semibold">{t("Como posso ajudar hoje?")}</h2><p className="mt-3 text-center text-sm leading-6 text-[#625E59]">{t("Seu contexto e suas mensagens continuam aqui.")}</p><div className="mt-6 grid gap-2">{[t("Resuma minhas últimas interações no app e as novidades da comunidade."),t("Quais posts devo acompanhar de acordo com meu perfil?"),t("Há algum Bench ou encontro novo que faça sentido para meu perfil?")].map(example=><button key={example} disabled={sending||loading||!loaded} onClick={()=>void ask(undefined,example)} className="min-h-11 rounded-lg border border-[#E6DED0] bg-white px-3 py-2 text-left text-sm leading-6 hover:bg-[#FAF7F1]">{example}</button>)}</div></div>:null}
    ...
    <div className="relative shrink-0 bg-white pb-3 pt-2">
      ...
      {error?<div role="alert" className="mb-3 flex flex-wrap items-center gap-2 rounded-xl bg-red-50 px-3 py-2 text-sm text-red-800"><p>{error}</p>{!loaded?<button onClick={()=>void load()} className="min-h-11 underline">{t("Tentar carregar novamente")}</button>:null}</div>:null}
