  ...
  return <section className="personal-agent mx-auto flex w-full max-w-5xl flex-col px-4 sm:px-6" aria-label="Seu agente pessoal">
    ...
    <div ref={scroll} className="min-h-0 flex-1 overflow-y-auto overscroll-contain py-6" aria-label="Histórico da conversa" tabIndex={0}>
      {loading?<p role="status" className="text-sm text-[#625E59]">Carregando sua conversa…</p>:<>
        ...
        {!turns.length&&!pendingQuestion&&loaded?<div className="mx-auto max-w-xl py-8 sm:py-14"><h2 className="text-center text-2xl font-semibold">Como posso ajudar hoje?</h2><p className="mt-3 text-center text-sm leading-6 text-[#625E59]">Continue por aqui sempre que precisar. Seu contexto e suas mensagens ficam nesta conversa.</p><div className="mt-6 grid gap-3">{['Quero avaliar um CLM para meu time. Por onde começo?','Quais vagas têm relação com meu perfil?','Como o OpenCLM pode ajudar minha operação?'].map(example=><button key={example} onClick={()=>{setQuestion(example);input.current?.focus()}} className="min-h-12 rounded-xl border border-[#CEC8BD] bg-white px-4 py-3 text-left text-sm leading-6">{example}</button>)}</div></div>:null}
