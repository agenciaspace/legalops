...
export default async function MembersPage({ searchParams }: { searchParams?: Record<string,string|string[]|undefined> }) {
  ...
  const [{data:raw,error},{data:{user}}]=await Promise.all([db.rpc('search_club_members',{filters}),db.auth.getUser()])
  ...
  return <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6">
    ...
    <form action="/community/members" className="mt-4 rounded-2xl border border-[#CEC8BD] bg-[#FAF7F1] p-4 sm:p-5">
      ...
      <details className="mt-3" open={active.some(([key])=>key!=='q')}><summary className="min-h-11 cursor-pointer py-3 text-sm font-semibold">{t('Filtros avançados')}</summary><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <label className="text-sm">{t('Validação')}<select name="verification" defaultValue={filters.verification} className={control}><option value="">{t('Todos')}</option><option value="verified">{t('Validado')}</option><option value="pending">{t('Em análise')}</option><option value="unverified">{t('Não validado')}</option></select></label>
      </div></details>
    ...
    <details className="mt-4 text-sm text-[#625E59]"><summary className="min-h-11 cursor-pointer py-3">{t('O que significa a validação?')}</summary><p className="leading-6">{t('Validado: a administração conferiu a identidade e o contexto profissional. Em análise: há uma solicitação na fila. Não validado: ainda não houve aprovação. O selo não certifica diplomas ou competências e não é requisito para participar da comunidade.')}</p><p className="mt-2 leading-6">{t('Localização e qualificações são opcionais e declaradas pelos membros. Perfis sem esses dados não aparecem nos filtros correspondentes.')}</p></details>
