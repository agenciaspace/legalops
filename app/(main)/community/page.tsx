...
export default async function CommunityPage({searchParams}:{searchParams?:{space?:string;q?:string;count?:string;post?:string}}) {
 ...
 return <div className="mx-auto w-full max-w-3xl px-4 py-6 sm:px-6">
  ...
  {selectedSpace && hasCommunityAccess ? <details className="mb-5 overflow-hidden rounded-2xl border border-[#D9D4CA] bg-white shadow-sm"><summary className="cursor-pointer list-none px-5 py-4 text-sm font-bold text-[#24231F] marker:hidden"><span className="mr-2 inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#FFF0E9] text-[#D9470F]">＋</span>Criar subtema em {COMMUNITY_CATEGORIES[selectedSpace].label}</summary><form action={createCommunitySubtopic} className="space-y-4 border-t border-[#ECECE8] bg-[#FAF7F1] p-5"><input type="hidden" name="category" value={selectedSpace} /><div><label className="text-[10px] font-black uppercase tracking-[0.12em] text-[#77746E]">Nome do subtema</label><input name="title" required minLength={3} maxLength={160} placeholder="Ex.: intake para contratos estratégicos" className="mt-2 min-h-11 w-full rounded-xl border border-[#D8D3C9] bg-white px-3 text-sm outline-none focus:border-[#D9470F]" /></div><div><label className="text-[10px] font-black uppercase tracking-[0.12em] text-[#77746E]">Objetivo da conversa</label><textarea name="description" maxLength={1000} rows={3} placeholder="O que vamos discutir?" className="mt-2 w-full rounded-xl border border-[#D8D3C9] bg-white p-3 text-sm outline-none focus:border-[#D9470F]" /></div><button className="min-h-11 rounded-xl bg-[#24231F] px-5 text-sm font-bold text-white">{t("Criar subtema")}</button></form></details> : null}
  {selectedSpace && subtopics?.length ? <section className="mb-5 rounded-xl border border-[#E1E1DD] bg-white p-4"><h2 className="text-sm font-bold">{t("Subtemas do fórum")}</h2><div className="mt-3 grid gap-2 sm:grid-cols-2">{subtopics.map(topic => <div key={topic.id} className="rounded-lg bg-[#F7F7F5] p-3"><p className="text-sm font-semibold">{topic.title}</p>{topic.description ? <p className="mt-1 text-xs text-[#716B65]">{topic.description}</p> : null}</div>)}</div></section> : null}
  ...
  <div className="space-y-4">
   {(feedEvents ?? []).map(event => <article key={`event-${event.id}`} className="rounded-xl border border-[#FFD8C8] bg-[#FFF8F4] p-4 sm:p-5">
    <div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#FFE3D7] text-[#D9470F]"><CalendarDays className="h-5 w-5" /></div><div className="min-w-0 flex-1"><p className="text-[9px] font-black uppercase tracking-[0.12em] text-[#D9470F]">{t("Registro de evento")}</p><h2 className="mt-1 text-[17px] font-extrabold leading-6 tracking-[-0.015em] text-[#252420]">{event.title}</h2><p className="mt-2 text-[13px] leading-[1.65] text-[#68655F]">{event.description}</p><p className="mt-2 text-xs font-semibold text-[#88857F]">{new Intl.DateTimeFormat(getClubLocale(),{dateStyle:'medium',timeStyle:'short',timeZone:'America/Sao_Paulo'}).format(new Date(event.starts_at))} · {event.location_label}</p><Link href={`/community/events/${event.slug}`} className="mt-3 inline-flex min-h-10 items-center rounded-lg bg-[#24231F] px-4 text-xs font-bold text-white">{t("Ver evento →")}</Link><div className="mt-2"><EventShare slug={event.slug} title={event.title} /></div></div></div>
   </article>)}
            {posts.map(post => {
                ...
                <article id={`post-${post.id}`} key={post.id} className="rounded-xl border border-[#E1E1DD] bg-white p-4 transition hover:border-[#D2D1CC] sm:p-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-sm font-medium text-[#48443E]">{author?.current_role || post.author_role || t("Membro do Club")}{author?.organization_name ? ` · ${author.organization_name}` : ''}</p>
