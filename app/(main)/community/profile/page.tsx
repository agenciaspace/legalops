...
export default async function CommunityProfilePage({ searchParams }: { searchParams?: { saved?: string; error?: string } }) {
    ...
    <div className="mx-auto w-full max-w-[980px] px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
      ...
      {searchParams?.saved ? (
        <div className="mt-5 flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-[11px] font-bold text-emerald-800">
          <Check className="h-4 w-4" /> Perfil salvo. Sua apresentação foi atualizada na comunidade.
        </div>
      ) : null}

      {searchParams?.error ? <p role="alert" className="mt-4 text-sm text-red-700">Não conseguimos salvar. Confira a foto, o contexto profissional e os campos obrigatórios.</p> : null}
      <div className="mt-5 grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_260px]">
        <form action={updateCommunityProfile} className="rounded-xl border border-[#E1E1DD] bg-white p-5 sm:p-6">
          <ProfilePhoto userId={user?.id ?? ""} path={profile?.avatar_path ?? null} name={profile?.full_name ?? "Seu perfil"} />
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="text-sm font-medium text-[#4D4B46] sm:col-span-2">Onde você trabalha e o que faz nesse contexto<textarea name="organization_description" required minLength={20} maxLength={700} rows={3} defaultValue={profile?.organization_description ?? ''} placeholder="Ex.: atuo no jurídico de uma empresa de tecnologia, cuidando de contratos comerciais e operações. Também pode descrever atuação autônoma ou transição profissional." className="mt-2 w-full rounded-xl border border-[#CEC8BD] px-3 py-3 text-base"/><span className="mt-2 block text-xs leading-5 text-[#625E59]">Esse contexto aparece em destaque nas suas publicações para os membros do Club.</span></label>
            <label className="text-[10px] font-extrabold text-[#4D4B46]">
              LinkedIn
          ...
          <div className="mt-5 flex justify-end border-t border-[#ECECE8] pt-4">
            <button className="rounded-lg bg-[#FF5C1A] px-4 py-2.5 text-[11px] font-extrabold text-white hover:bg-[#E84D10]">Salvar perfil</button>
          </div>
        </form>
