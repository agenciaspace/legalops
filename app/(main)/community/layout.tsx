...
export default async function CommunityLayout({children}:{children:React.ReactNode}) {const db=await createServerSupabaseClient();const {data:{user}}=await db.auth.getUser();const {data:member}=await db.from('community_members').select('club_pro_status,club_pro_expires_at').eq('user_id',user?.id??'').maybeSingle();return <div className="club-shell min-h-[calc(100dvh-4rem)] bg-[#F3F0E8] text-[#24231F]"><div className="flex min-h-[calc(100dvh-4rem)]"><CommunityTabs/><div className="club-content min-w-0 flex-1"><ClubPwa/><ClubWelcomeNotice/>{children}</div></div><AgentBubble hasPro={hasClubProAccess(member)}/></div>}
