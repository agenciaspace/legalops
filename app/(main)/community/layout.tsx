...
export default async function CommunityLayout({children}:{children:React.ReactNode}) {
  ...
  const {data:member}=await db.from('community_members').select('club_pro_status,club_pro_expires_at,profile_verification_status').eq('user_id',user?.id??'').maybeSingle()
  return <div className="club-shell min-h-[calc(100dvh-4rem)] bg-[#F3F0E8] text-[#24231F]">
    <div className="flex min-h-[calc(100dvh-4rem)]">
      ...
      <div className="club-content min-w-0 flex-1">
        <ClubPwa/>
        {!user && <div className="flex justify-end px-4 pt-4"><ClubLanguageSelect /></div>}
