...
export default function SetPasswordPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (password.length < 8 || password !== confirmation) {
      setError(password.length < 8 ? t("Use pelo menos 8 caracteres.") : t("As senhas não conferem."))
      return
    }
    setLoading(true)
    setError(null)
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#F5F1E8] p-4">
      <div className="w-full max-w-md">
        <BrandLogo suffix="club" className="mb-8 flex flex-col items-center" titleClassName="text-3xl font-semibold tracking-tight" subtitle={t("Ative seu acesso")} />
