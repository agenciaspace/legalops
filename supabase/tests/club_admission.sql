...
begin
  ...
  assert exists(select 1 from public.community_members where user_id=auth.uid() and club_plan='free' and club_pro_status='inactive' and profile_verification_status='unverified'), 'Unexpected plan or verification escalation';
...
reset role;
update public.community_members set profile_verification_status='rejected' where user_id='11111111-1616-4616-8616-111111111111';
...
do $$ begin
  assert exists(select 1 from public.community_members where user_id='11111111-1616-4616-8616-111111111111' and profile_verification_status='rejected'), 'Profile edits must preserve moderation';
end $$;
select 'All admission, entitlement and moderation assertions passed' as result;
rollback;
