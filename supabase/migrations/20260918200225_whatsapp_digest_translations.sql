alter table public.club_translation_sources add column whatsapp_summary_id uuid unique references public.club_whatsapp_summaries(id) on delete cascade;
alter table public.club_translation_sources drop constraint translation_one_original, drop constraint translation_original_id, drop constraint translation_original_kind;
alter table public.club_translation_sources add constraint translation_one_original check(num_nonnulls(post_id,comment_id,event_id,topic_id,member_id,summary_id,resource_id,whatsapp_summary_id)=1),add constraint translation_original_id check(entity_id=coalesce(post_id,comment_id,event_id,topic_id,member_id,summary_id,resource_id,whatsapp_summary_id)),add constraint translation_original_kind check((entity_type='post' and post_id is not null) or (entity_type='comment' and comment_id is not null) or (entity_type='event' and event_id is not null) or (entity_type='topic' and topic_id is not null) or (entity_type='member' and member_id is not null) or (entity_type='summary' and summary_id is not null) or (entity_type='resource' and resource_id is not null) or (entity_type='whatsapp_summary' and whatsapp_summary_id is not null));
create policy translations_follow_whatsapp_summary on public.club_translation_sources for select to authenticated using(whatsapp_summary_id is not null and exists(select 1 from public.club_whatsapp_summaries s where s.id=whatsapp_summary_id));
...
begin
  ...
  insert into public.club_translation_sources(entity_type,entity_id,post_id,comment_id,event_id,topic_id,member_id,summary_id,resource_id,whatsapp_summary_id,payload,locale_hint)
  values(tg_argv[0],source_key,
    ...
    case when tg_argv[0]='member' then source_key end,case when tg_argv[0]='summary' then source_key end,case when tg_argv[0]='resource' then source_key end,case when tg_argv[0]='whatsapp_summary' then source_key end,selected,hint)
...
create trigger queue_translation after insert or update on public.club_whatsapp_summaries for each row execute function private.queue_club_translation('whatsapp_summary','id','title,summary,key_points');
