...
const state=vi.hoisted(()=>({user:null as any,member:null as any,profile:null as any,admin:vi.fn()}))
vi.mock('@/lib/supabase-server',()=>({createServerSupabaseClient:async()=>({auth:{getUser:async()=>({data:{user:state.user}})},from:(table:string)=>{const q={select:()=>q,eq:()=>q,maybeSingle:async()=>({data:table==='account_profiles'?state.profile:state.member})};return q}})}))
...
beforeEach(()=>{vi.clearAllMocks();state.user=null;state.member=null;state.profile=null})
it('blocks anonymous access but lets a signed-in applicant reach their own photo flow',async()=>{
 ...
 state.user={id:uid};state.member={club_access_status:'inactive'};state.profile={avatar_path:null}
 expect((await POST(request())).status).toBe(415)
 expect((await GET(request(),{params:{id:uid}})).status).toBe(404)
 expect(state.admin).not.toHaveBeenCalled()
})
it('rejects nonimages, oversized payloads and paths owned by somebody else',async()=>{
 state.user={id:uid};state.member={club_access_status:'active'};state.profile={avatar_path:null}
...
it('does not let an active member remove the required photo',async()=>{
 state.user={id:uid};state.member={club_access_status:'active'};state.profile={avatar_path:`${uid}/11111111-1111-4111-8111-111111111111.jpg`}
