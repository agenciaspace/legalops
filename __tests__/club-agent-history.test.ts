...
vi.mock('@/lib/supabase-server',()=>({createServerSupabaseClient:async()=>({auth:{getUser:async()=>({data:{user:state.user}})},from:(table:string)=>{const data=()=>table==='community_members'?{club_access_status:'active',club_pro_status:'active'}:table==='club_agent_turns'?state.rows:null;const q:any={};for(const m of ['select','eq','order','range'])q[m]=(...args:any[])=>{state.calls.push([table,m,...args]);return q};q.maybeSingle=async()=>({data:data()});q.then=(resolve:any)=>resolve({data:data()});return q}})}))
