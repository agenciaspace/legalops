...
class SummaryTests(unittest.TestCase):
    ...
    def test_source_filter_and_transcription(self):
        ...
        db.execute('create table messages(wm_id,ts,sender_jid,text,msg_id,chat_jid,owner,deleted)')
        ...
        base=[1,end.timestamp()-1,'author','Message','m1',runner.GROUP,runner.OWNER,0]
        rows=[base]
        for index,changes in enumerate([{5:'other-group'},{6:'other-owner'},{7:1},{1:end.timestamp()},{1:(end-runner.DAY).timestamp()-1},{3:runner.PREFIX+' body'},{3:''},{3:''},{4:'m1'}],start=2):
            row=base.copy();row[0]=index;row[4]=f'm{index}'
        ...
        self.assertEqual(messages[0]['author'],'Participante 1')
